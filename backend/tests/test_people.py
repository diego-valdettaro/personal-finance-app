...
class TestPersonCreation:
    ...
    def test_create_person_success(self, client, sample_user):
        ...
        response = client.post(f"/users/{sample_user.id}/people/", json=person_data)
    ...
    def test_create_person_is_me_true(self, client, sample_user):
    ...
    def test_create_person_default_is_me(self, client, sample_user):
    ...
    def test_create_person_missing_required_fields(self, client, sample_user):
        """Test person creation with missing required fields."""
        # Missing name
        response = client.post(f"/users/{sample_user.id}/people/", json={
            "is_me": False,
            "user_id": sample_user.id
        })
    ...
    def test_create_person_name_too_long(self, client, sample_user):
    ...
    def test_create_person_duplicate_name(self, client, sample_user):
    ...
    def test_create_person_duplicate_is_me(self, client, sample_user):
    ...
    def test_create_person_empty_name(self, client, sample_user):
...
class TestGetPeople:
    ...
    def test_get_all_people_empty(self, client, sample_user):
        """Test getting all people when no people exist."""
        response = client.get(f"/users/{sample_user.id}/people/")
        assert response.status_code == 200
        assert response.json() == []
    
    def test_get_all_people_multiple(self, client, sample_user):
    ...
    def test_get_person_success(self, client, sample_user):
        ...
        # Get person
        response = client.get(f"/users/{sample_user.id}/people/{person['id']}")
        assert response.status_code == 200
        data = response.json()
        assert data["id"] == person["id"]
        assert data["name"] == person["name"]
    
    def test_get_person_not_found(self, client, sample_user):
        """Test getting a non-existent person."""
        response = client.get(f"/users/{sample_user.id}/people/99999")
        assert response.status_code == 404

class TestUpdatePerson:
    """Test cases for updating people"""
    
    def test_update_person_name_only(self, client, sample_user):
        ...
        response = client.patch(f"/users/{sample_user.id}/people/{person['id']}", json=update_data)
        assert response.status_code == 200
        data = response.json()
        assert data["name"] == "Updated Name"
        assert data["is_me"] == person["is_me"]  # Should remain unchanged
    
    def test_update_person_is_me(self, client, sample_user):
    ...
    def test_update_person_multiple_fields(self, client, sample_user):
    ...
    def test_update_person_duplicate_name(self, client, sample_user):
        ...
        response = client.post(f"/users/{sample_user.id}/people/", json=person1_data)
        person1 = response.json()
        response = client.post(f"/users/{sample_user.id}/people/", json=person2_data)
        person2 = response.json()
        
        # Try to update person2 with person1's name
        update_data = {"name": person1["name"]}
        response = client.patch(f"/users/{sample_user.id}/people/{person2['id']}", json=update_data)
        assert response.status_code == 409
    
    def test_update_person_duplicate_is_me(self, client, sample_user):
    ...
    def test_update_person_not_found(self, client, sample_user):
        """Test updating a non-existent person."""
        update_data = {"name": "Updated Name"}
        response = client.patch(f"/users/{sample_user.id}/people/99999", json=update_data)
        assert response.status_code == 404
    
    def test_update_person_same_values(self, client, sample_user):
...
class TestDeactivatePerson:
    ...
    def test_deactivate_person_success(self, client, sample_user):
        ...
        # Deactivate person
        response = client.patch(f"/users/{sample_user.id}/people/{person['id']}/deactivate")
    ...
    def test_deactivate_person_not_found(self, client, sample_user):
        """Test deactivating a non-existent person."""
        response = client.patch(f"/users/{sample_user.id}/people/99999/deactivate")
        assert response.status_code == 404

class TestActivatePerson:
    """Test cases for activating people"""
    
    def test_activate_person_success(self, client, sample_user):
        ...
        # Activate person
        response = client.patch(f"/users/{sample_user.id}/people/{person['id']}/activate")
        assert response.status_code == 204
    ...
    def test_activate_person_not_found(self, client, sample_user):
        """Test activating a non-existent person."""
        response = client.patch(f"/users/{sample_user.id}/people/99999/activate")
        assert response.status_code == 404

class TestPersonValidation:
    """Test cases for person validation and business rules"""
    
    def test_person_name_maximum_length(self, client, sample_user):
    ...
    def test_person_name_too_long(self, client, sample_user):
    ...
    def test_special_characters_in_name(self, client, sample_user):
...
class TestPersonEdgeCases:
    ...
    def test_multiple_people_same_user(self, client, sample_user):
    ...
    def test_person_with_me_flag(self, client, sample_user):
...
class TestPersonIntegration:
    ...
    def test_person_lifecycle(self, client, sample_user):
        ...
        response = client.patch(f"/users/{sample_user.id}/people/{person_id}", json=update_data)
        assert response.status_code == 200
        assert response.json()["name"] == "Updated Lifecycle Person"
        
        # Deactivate person
        response = client.patch(f"/users/{sample_user.id}/people/{person_id}/deactivate")
        assert response.status_code == 204
        ...
        # Activate person
        response = client.patch(f"/users/{sample_user.id}/people/{person_id}/activate")
        assert response.status_code == 204
    ...
    def test_person_me_constraint(self, client, sample_user):
