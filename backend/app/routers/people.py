...
def get_people(db: Session = Depends(get_db)):
    return crud_people.get_all_people(db)

# Get a person
@router.get("/{person_id}", response_model=schemas.PersonOut)
def get_person(person_id: int, db: Session = Depends(get_db)):
    person = crud_people.get_person_by_id(db, person_id)
    if not person:
...
def update_person(person_id: int, person: schemas.PersonUpdate, db: Session = Depends(get_db)):
    return crud_people.update_person_by_id(db, person_id, person)
...
def deactivate_person(person_id: int, db: Session = Depends(get_db)):
    ...
    person = crud_people.get_person_any_status(db, person_id)
    if not person:
        raise HTTPException(status_code=404, detail="Person not found")
    crud_people.deactivate_person(db, person.user_id, person_id)
    return None
...
def activate_person(person_id: int, db: Session = Depends(get_db)):
    ...
    if not person:
        raise HTTPException(status_code=404, detail="Person not found")
    crud_people.activate_person(db, person.user_id, person_id)
    return None